...
class ProdutoAuxiliarController extends CommonController {

	private $modulos = array();
	private $estados = array();
	private $classe = 'ProdutoAuxiliar';
	private $breadcrumb = array();
	private $titulo_principal = '';	
	
	public function __construct() {
		$produtoAuxiliar = new ProdutoAuxiliar();
		$produtoAuxiliar->common = new CommonController();	
		$modulos = $produtoAuxiliar->common->getModulos();
		$estados = $produtoAuxiliar->common->getEstados();
	...
	public function listar() {
		...
		$num_registros = $produtoAuxiliar->listarTodosTotal();
		...
		$arrCamposBusca = array('Cd_Prod_Aux'	=> 'Descrição');	
		...
		$classe = $this->classe;
		require './src/View/ProdutoAuxiliar/produto_auxiliar_listar.php';
	}
	...
	public function editar($handle) {
		...
		if (isset($_POST) && !empty($_POST)) {
			$retorno = $produtoAuxiliar->editarProdutoAuxiliar($_POST);
			if ($retorno) {
		...
		$produtosAuxiliar = new ProdutoAuxiliar();
		$produtosAuxiliar = $produtoAuxiliar->listarProdutoAuxiliar($handle);
		$titulo_principal = $this->titulo_principal;
		$breadcrumb = $this->breadcrumb;		
		$modulos = $this->modulos;
		$classe = $this->classe;
		require './src/View/ProdutoAuxiliar/produto_auxiliar_form.php';
	}
	...
	public function cadastrar() {
		...
		if (isset($_POST) && !empty($_POST)) {
			$retorno = $produtoAuxiliar->cadastrarProdutoAuxiliar($_POST);
			if ($retorno) {
				$msg_sucesso = 'ProdutoAuxiliar cadastrado com sucesso.';
			}
			$produtosAuxiliar = $produtoAuxiliar->listarProdutoAuxiliar($retorno);
		} else {
			$produtosAuxiliar = array($produtoAuxiliar);
		}		
