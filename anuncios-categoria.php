...
$arrAnunciosCategoria = MaterialController::buscaAnuncioPorCategoria($id_categoria);
$arrAnunciosCategoria = json_decode($arrAnunciosCategoria, true);

$quantidade_anuncios_categoria = count($arrAnunciosCategoria);

$arrCategoria = CategoriaController::buscaCategoria($id_categoria);
$arrCategoria = json_decode($arrCategoria, true);

$arrCategorias = CategoriaController::buscaArvoreCategorias();
$arrCategorias = json_decode($arrCategorias, true);

$id_categoria_pai = $arrCategoria[0]['id_categoria_pai'];
if ($arrCategoria[0]['id_categoria_pai'] == 0) {
	$id_categoria_pai = $id_categoria;
}

$nome_categoria = $arrCategoria[0]['descricao'];
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-8">
                    ...
                    <!--Related Ads-->
                    <div id="relatedAds">
                        <h4 class="inner-heading"><?= str_pad($quantidade_anuncios_categoria, 2, '0', STR_PAD_LEFT);;?> ANÚNCIOS</h4>
                        <div class="row">
									...
									<?php
										if (is_array($arrAnunciosCategoria)) {
											foreach ($arrAnunciosCategoria as $anuncio) {
												...
												echo sprintf('<div class="ads-title"><p><a href="#">%s</a></p></div>', $anuncio['descricao_cor']);
						...
						<?php } else { ?>
							<h4> Nenhum anúncio encontrado para a categoria :-( </h4>
						<?php } ?>
                    </div>
                <div class="col-md-4">
                    <div class="sidebar">
                        <div class="side-widget clearfix">
                            <h4 class="inner-heading"><?= $arrCategorias[$id_categoria_pai]['nome'];?></h4>
							<?php 
								foreach ($arrCategorias[$id_categoria_pai]['subcategorias'] as $subcategoria) {
									echo '<div class="subcat-widget">';
									echo '<p>'.$subcategoria['descricao'].'</p>';
									echo '</div>';
								}
							?>
